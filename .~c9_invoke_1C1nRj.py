...
def buy():
    ...
    if request.method ==  "POST":
        ...
        bb = db.execute("SELECT cash FROM users WHERE id =?", user_id)  
        print(bb)  
        ...
        db.execute("INSERT INTO  purchase (users_id, symbol, price, share, transaction_time) VALUES(?, ?, ?, ?, datetime('now'))", user_id, stock["symbol"], price, int(shares))
        print(type(shares))
        print(shares)
...
def sell():
    ...
    if request.method ==  "POST":
        ...
        return render_template("sold.html", symbol=symbols, shares=shares)
    else:
